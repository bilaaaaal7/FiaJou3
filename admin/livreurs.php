...
if (isset($_GET['supprimer'])) {
    ...
    $nbAssign = $pdo->prepare("SELECT COUNT(*) FROM orders WHERE assigned_driver_id = ?");
    $nbAssign->execute([$id]);
    if ($nbAssign->fetchColumn() > 0) {
        $message = "Impossible de supprimer : ce livreur est encore affecté à des commandes."; $messageType = "error";
    } else {
...
$stmt = $pdo->prepare("
    SELECT users.id, users.email, users.actif, profiles.prenom, profiles.nom, profiles.telephone,
           (SELECT COUNT(*) FROM orders WHERE orders.assigned_driver_id = users.id AND orders.statut NOT IN ('livree','annulee')) AS livraisons_actives
    FROM users INNER JOIN profiles ON users.id = profiles.user_id
    WHERE profiles.role = ?
...
<div class="two-col">
    ...
    <div class="panel">
        <h2>Livreurs (<?php echo count($livreurs); ?>)</h2>
        <?php if (empty($livreurs)) { ?>
            <div class="empty-state">Aucun livreur pour le moment.</div>
        <?php } else { ?>
        <div class="table-wrap">
            <table class="data-table">
                <tr><th>Nom</th><th>Email</th><th>Téléphone</th><th>Livraisons en cours</th><th>Statut</th><th>Actions</th></tr>
                <?php foreach ($livreurs as $l) { ?>
                <tr>
                    <td><?php echo htmlspecialchars($l['prenom'].' '.$l['nom']); ?></td>
                    <td><?php echo htmlspecialchars($l['email']); ?></td>
                    <td><?php echo htmlspecialchars($l['telephone']); ?></td>
                    <td><?php echo $l['livraisons_actives']; ?></td>
                    <td><?php echo $l['actif'] ? '<span class="badge-yes">● Actif</span>' : '<span class="badge-no">● Désactivé</span>'; ?></td>
                    <td class="actions-cell">
                        <a class="btn btn-outline btn-sm" href="?modifier=<?php echo $l['id']; ?>">Modifier</a>
                        <a class="btn <?php echo $l['actif'] ? 'btn-danger' : 'btn-outline'; ?> btn-sm" href="?toggle=<?php echo $l['id']; ?>">
                            <?php echo $l['actif'] ? 'Désactiver' : 'Réactiver'; ?>
                        </a>
                        <a class="btn btn-danger btn-sm" href="?supprimer=<?php echo $l['id']; ?>" data-confirm="Supprimer ce livreur ?">Supprimer</a>
                    </td>
                </tr>
                <?php } ?>
            </table>
        ...
        <form method="POST">
            ...
            <div class="form-actions">
                <?php if ($idModifier) { ?>
                    ...
                    <a href="livreurs.php" class="btn btn-outline">Annuler</a>
                <?php } else { ?>
